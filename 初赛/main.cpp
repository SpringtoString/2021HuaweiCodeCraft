...
// 虚拟机需求从大到小排列，依次满足需求
// 增加迁移操作,迁移时服务器按使用率排序,需要迁移的服务器内的虚拟机升序排序,放置虚拟机时按放置后最大利用率放
// 修改利用率公式
//#define UPLOAD
...
/*虚拟机迁移比率*/
double MigrateRate=0.005;

/*输入数目变量*/
int serverNum,vmNumber;  //服务器类型数目,虚拟机类型数量
int requestdays = 0, dayRequestNumber = 0;   // 天数，每天请求量
...
public:
    ...
    /*处理创建虚拟机操作*/
    /*int createVM(int vmid){
        int success = -1;
        int vmtypeidx = vmid2vmtypeidx[vmid];
        //cout<<"vmtypeidx = "<<vmtypeidx<<endl;
        VM& v = AllVm[vmtypeidx];
        //cout<<"isDouble= "<<v.isDouble<<endl;
        int cpuCores=v.cpuCores, memorySize=v.memorySize;

        for(int i=0;i<PurServerCnt;i++){
            SERVER& server = PurServer[i];
            int m = ismatch(server,v);
            if(m!=-1){
                success=1;
                server.runvmcnt++;
                ...
                if(m==0) {
                    server.A_cpuCores=server.A_cpuCores-cpuCores/2;server.B_cpuCores=server.B_cpuCores-cpuCores/2;
                    server.A_memorySize=server.A_memorySize-memorySize/2;server.B_memorySize=server.B_memorySize-memorySize/2;
                    vmid2loc[vmid]=vector<int>{i,0};
                }
                if(m==1) {
                    server.A_cpuCores=server.A_cpuCores - cpuCores;
                    server.A_memorySize=server.A_memorySize- memorySize;
                    vmid2loc[vmid]=vector<int>{i,1};
                }
                if(m==2) {
                    server.B_cpuCores=server.B_cpuCores - cpuCores;
                    server.B_memorySize=server.B_memorySize - memorySize;
                    vmid2loc[vmid]=vector<int>{i,2};
                }
                break;
            }
        }
...
int main() {
...
#endif // TEST
    vector<vector<string> > BestExpanRes,BestMigrateRes,BestCreateVmRes;
    ...
    printf("not satify\n");
        ...
        vector<vector<string>>& BestExpanRes = solver.ExpanRes;
        vector<vector<string>>& BestMigrateRes = solver.MigrateRes;
        vector<vector<string>>& BestCreateVmRes = solver.CreateVmRes;
#ifdef UPLOAD
        for(int i=0;i<requestdays;i++){
            for(string s:BestExpanRes[i])
                cout<<s;
            for(string s:BestMigrateRes[i])
                cout<<s;
            for(string s:BestCreateVmRes[i])
                cout<<s;
    }
#endif // UPLOAD
    }
